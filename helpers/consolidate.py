...
def _push_to_cache_branch(filepath: str, now: datetime):
    ...
    try:
        ...
        result = subprocess.run(["git", "fetch", "origin", branch], capture_output=True)
        ...
        subprocess.run(
            ["git", "push", remote_url, f"HEAD:{branch}", "--force"], check=True
        )
        ...
        print(
            f"  ⚠ Cache push failed: {e} — intraday comparison will still work within same run"
        )
...
def _pull_from_cache_branch(results_dir: str):
    ...
    try:
        ...
        if not files:
            print(
                f"  ℹ No cache files found for today ({today_str}) — first run of day."
            )
            return
        ...
        for f in files:
            ...
            dest = os.path.join(
                results_dir, os.path.basename(f).replace(f"{today_str}_", "")
            )
...
def check_and_alert(
    ...
    print(
        f"     Combo ROI:  {best_prev_roi:.2f}% → {curr_roi:.2f}% ({combo_improvement:+.2f}%)"
    )
    ...
    for s in improved_stocks:
        print(
            f"     {s['ticker']:20s} ₹{s['prev_price']} → ₹{s['curr_price']} ({s['pct_drop']:+.2f}%)"
        )
